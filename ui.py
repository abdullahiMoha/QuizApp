...
class QuizInterface:
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.get_next_question()  # The call of populating the question into box

        self.window.mainloop()  # This line should be at last line.

    def get_next_question(self):
        self.canvas.config(bg = "white")
        if self.quiz.still_has_questions():
            self.score_label.config(text = f"Score : {self.quiz.score}/{Q_NUM}")
            q_text = self.quiz.next_question()
            self.canvas.itemconfig(self.question_text, text = q_text)
        else:
            self.canvas.itemconfig(self.question_text,
                                   text = f"The Quiz is OVER\n Your Score : {self.quiz.score}/{Q_NUM}")
